...
export default function HeaderBar({ score, total, paused, onPauseToggle, onHome, onReset }) {
      ...
      <div className="bg-white rounded-2xl border-3 border-purple-200 p-4 md:p-5 shadow-md">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 md:gap-4">
            <button
              onClick={onHome}
              className="group flex items-center justify-center w-12 h-12 md:w-14 md:h-14 bg-pink-400 hover:bg-pink-500 border-2 border-pink-500 rounded-xl shadow-sm hover:shadow-md transform hover:scale-110 transition-all duration-200"
            >
              <span className="text-2xl md:text-3xl">🏠</span>
            </button>
            
            <div className="flex flex-col">
              <p className="text-xs md:text-sm text-gray-500 font-bold uppercase tracking-wide">
                Score
              </p>
              <div className="flex items-baseline gap-1.5">
                ...
                <span className="text-lg md:text-xl text-gray-400 font-bold">
                  / {total}
                </span>
              </div>
            </div>
              ...
              {[...Array(3)].map((_, i) => (
                <span 
                  ...
                  className={`text-xl md:text-2xl transition-all duration-300 ${
                    i < stars ? "scale-110" : "opacity-30 grayscale"
                  }`}
                >
                  {i < stars ? "🌟" : "⭐"}
                </span>
