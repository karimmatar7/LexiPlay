...
export default function FinalWordBuilder({ user, setUser }) {
  ...
  const handleLetterClick = (letter) => {
    ...
    const correctLetter = currentWord.correct[nextIndex]
    
    if (letter === correctLetter) {
      const updated = [...selected, letter]
      setSelected(updated)
      playCuteBeep(1000, 0.08)
      if (updated.length === currentWord.correct.length) {
  ...
  if (showResetModal) return <ResetConfirmationModal onConfirm={resetGame} onCancel={() => setShowResetModal(false)} cancelText="Annuleren" />

  const floatingLetters = currentWord ? shuffle(currentWord.correct.split("")) : []
    ...
    <>
      ...
      >
        <div className="text-center space-y-4 md:space-y-8">
          ...
          <WordDisplay selected={selected} totalLetters={currentWord?.correct.length || 0} shakeWrong={shakeWrong} />
          ...
          <div className="text-center">
            <div className="inline-flex items-center gap-2 bg-white px-4 md:px-6 py-2 md:py-3 rounded-full shadow-lg border-2 border-purple-200">
              ...
              <span className="text-base md:text-lg font-bold text-gray-700">
                Woord <span className="text-purple-600">{currentIndex + 1}</span> / {TOTAL_WORDS}
              </span>
            </div>
          </div>
        </div>
